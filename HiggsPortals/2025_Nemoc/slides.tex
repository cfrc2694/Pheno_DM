...
\begin{frame}{Statistical Significance}
    ...
    \begin{equation}
        \kappa = \frac{\sum_i s_i w_i}{\sqrt{\sum_i (s_i + b_i + \delta_{sys}^2)^2 w_i^2}}
        =\frac{\sum_i \sigma_{s_i} \epsilon_{s_i} w_i}{\sqrt{\sum_i (\sigma_{s_i}\epsilon_{s_i} + \sigma_{b_i}\epsilon_{b_i} + \delta_{\sigma_{sys}}^2)^2 w_i^2}}\sqrt{\textcolor{blue}{\mathcal L}}
    \end{equation}
    We can
    \begin{itemize}
...
\begin{frame}{Projections on the Higgs-to-invisible decay}
    ...
    \begin{itemize}
        \item The current limit on the Higgs-to-invisible decay is $\text{BR}(H \to \text{inv}) < 0.145$.
